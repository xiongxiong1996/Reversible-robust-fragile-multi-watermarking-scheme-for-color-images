...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [bool,local_map] = dq_preEmbedingWatermarking(hostImg,block_size,w_sequence,sorted_index,b_num_p,T,G,n_level)
...
% 对宿主图像进行分层
[L L n] = size(hostImg); % 获取宿主图像的size
...
for i=1:b_num_p % 对预计要嵌入的块进行嵌入，看能不能将水印序列全部嵌入
	if w_index<=w_len
		...
		% %%%   R层预嵌入   %%%
		% %%%%%%%%%%%%%%%%%%%%%
		block_r=r_l(x1:x2,y1:y2); % r层要嵌入的块
		...
		% %%%   G层预嵌入   %%%
		% %%%%%%%%%%%%%%%%%%%%%
		block_g=g_l(x1:x2,y1:y2); % g层要嵌入的块
		% block_g = imcrop(g_l,[x1,y1,block_size-1,block_size-1]);
		[ll_g, lh_g, hl_g, hh_g] = dq_iwtTransfrom(block_g,n_level); % g层要嵌入的块进行IWT分解
		[ll_gp,bool_g]=qyh_blockEmbed(ll_g,block_size/2,T,G); % 对G层进行嵌入，其实这里的bool_g没用，我们要看最后逆变换后是否溢出
		w_index=w_index+1; % 水印序列编号+1
		% %%%%%%%%%%%%%%%%%%%%%
		% %%  判断是否溢出   %%
		% %%%%%%%%%%%%%%%%%%%%%
		[w_block_r] = dq_inverIwtTransform(ll_rp,lh_r, hl_r, hh_r,n_level); % 对R层块嵌入后逆变换
		[w_block_g] = dq_inverIwtTransform(ll_gp,lh_g, hl_g, hh_g,n_level); % 对G层块嵌入后逆变换
		[m n] = size(w_block_r); % 获取嵌入块的大小
		b=1; % 默认不溢出，可以进行嵌入
		for i=1:m
			for j=1:n
				if w_block_r(i,j)>255 || w_block_r(i,j)<0 || w_block_g(i,j)>255 || w_block_g(i,j)<0 
		...
		if b==1 % 如果不溢出，进行嵌入
			...
			w_index=w_index-2; % 水印序列编号-2 R,G均不进行嵌入
		end % if
	end % for
end % for
% %%%%%%%%%%%%%%%%%%%%%
